...
class GetData:
    def __init__(self, csv, encoding):
        self.df = pd.read_csv(csv, encoding=encoding)
    ...
    def df_drop_and_modify_columns(self, features_to_drop, features_to_modify):
        data = self.df.drop(columns=[features_to_drop], axis=1)
        data = self.df.rename(columns=features_to_modify)
        return data
